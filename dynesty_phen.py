...
parser = argparse.ArgumentParser(description="Run cosmological nested sampling.")
parser.add_argument("--data", type=str, default="all", 
                    choices=["all", "CMB_only", "no_desi", "desi_only"],
                    help="Datasets to use for nested sampling")
parser.add_argument("--function", type=str, default="exp", 
                    choices=["exp", "sech", "tanh", "polynomial"],
                    help="Phenomenological function type")

args = parser.parse_args()

# --- Map arguments to your existing variables ---
nested_sampling_data = args.data
phenomenological_function = args.function

# Keep the rest of your boolean flags as they are, or add more arguments for them!
compute_minimum_chi2 = True
...
redshifts = np.loadtxt('Pantheon+SH0ES.dat', usecols=2, unpack=True)
z_SH0ES = redshifts[(redshifts > 0.023) & (redshifts < 0.15)]
z_SH0ES = np.sort(z_SH0ES)
...
def tanh_z1_by_z(z, z1, threshold=20.0):
    if z < 1e-10: return 1.0
    x = z1 / z
    if x > threshold: return 1.0
    return np.tanh(x)

tanh_z1_by_z_vec = np.vectorize(tanh_z1_by_z, excluded=['threshold'])

def integrand_rec(u, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1):
    z = np.sinh(u)
    H = Hubble_factor(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1)
    return np.cosh(u) / H

def integrand(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1):
    H = Hubble_factor(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1)
    return 1 / H

from scipy.integrate import quad

def comoving_distance_arcsinh(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1, z_star):
    integral, _ = quad(integrand_rec, 0, np.arcsinh(z_star), args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1))
    return c * integral  # Return in metres

def comoving_distance(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1, z):
    integral, _ = quad(integrand, 0, z, args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1))
    return c * integral  # Return in metres

comoving_distance_vec = np.vectorize(comoving_distance, excluded=['Omega_bc', 'Omega_gamma', 'Omega_nu', 'Omega_nu_ur', 'a_nr_sq', 'H0', 'A', 'z_1'])

def model_predictions(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1, r_d):
    DM_array = comoving_distance_vec(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1, z)
    DH_array = c / Hubble_factor(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, H0, A, z_1) # DH in Mpc
...
def theta_star(w_b, w_bc, h, A, z_1):
    ...
    DM_rec = comoving_distance_arcsinh(w_bc / h**2, w_gamma / h**2, w_nu / h**2, w_nu_ur / h**2, a_nr_sq, h * H_100, A, z_1, z_rec)
...
def log_likelihood(theta, use_desi=True, use_h0=True, use_cc=True, use_cmb=True, desi_only=False):
    ...
    if use_h0:
        z = z_SH0ES
        H = Hubble_factor(z, omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, h*H_100, A*H_100/100, z_1)
        H = H*100/H_100  # Convert to km/s/Mpc

        # Cosmography fit assuming q0 = -0.55 as SH0ES did
        q0 = -0.55
    ...
    if use_cc:
        ...
        H_z_CC_predicted = H_z_CC_predicted * 100 / H_100  # Convert to km/s/Mpc
...
def chi_squared(theta, print_output=False):
    ...
    Omega_Lambda = 1 - omega_m - w_gamma/h**2 - w_nu/h**2
    #H0_Planck = H0 - A
    predictions = model_predictions(z_DESI, omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, h*H_100, A*H_100/100, z_1, r_d)
    diff = predictions - Data_DESI
    chi2_DESI = diff @ C_inv_total @ diff
    ...
    #chi2_noDESI = chi2_H0 + chi2_CMB + chi2_SNII + chi2_SBF + chi2_maser + chi2_CC
    #return chi2_noDESI
    #return chi2_CMB
...
def minimize_chi2():
    ...
    # Initial guess: [w_b, omega_m, H0, A, z_1]
    # Make sure the guess is safely inside your prior bounds!
    initial_guess = [0.0223, 0.3, 72.0, 1.0, 1.0] 

    def objective_function(theta):
        # 1. Check if parameters are physical using your existing log_prior
        # log_prior returns 0 for valid, -inf for invalid
        if not np.isfinite(log_prior(theta)):
            return 1e50  # Return infinite penalty so optimizer retreats
        
        # 2. If valid, calculate actual chi_squared
        try:
            return chi_squared(theta, print_output=False)
        except Exception:
            # Catch math errors (like integration failures) and penalize
            return 1e50

    # We use the wrapper 'objective_function' instead of 'chi_squared' directly
...
if perform_nested_sampling:
    ...
    from dynesty import utils as dyfuncj
    ...
    # Save the samples, weights, and log_posteriors to a .npz file
    # This will now use the name passed from Slurm!
    #filename = "dynesty_results_phen_" + phenomenological_function + "_final.npz"
    #np.savez(filename, samples=samples, weights=weights, log_posteriors=log_posteriors)
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_phen_"+phenomenological_function+"_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "all":
        np.savez("dynesty_results_phen_"+phenomenological_function+"_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_phen_"+phenomenological_function+"_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_phen_"+phenomenological_function+"_final_DESI_only.npz", samples=samples, weights=weights)

def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0, A, z_1):
    h = H0 / 100.0
    def integrand(a):
        z = 1 / a - 1
        H = a * Hubble_factor(z, Omega_bc, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, H0, A, z_1)
        return 1.0 / H

    integral, _ = quad(integrand, 0, 1)
    t_0 = integral # in km/s/Mpc
...
if compute_Universe_age:
    try:
        data = np.load("dynesty_results_phen_"+phenomenological_function+"_final.npz")
        samples = data['samples']
        weights = data['weights']
    except FileNotFoundError:
        raise FileNotFoundError("Nested sampling results file not found. Please run nested sampling first.")

    ages = np.array([age_of_universe(theta[1], w_gamma, w_nu, w_nu_ur, a_nr_sq, theta[2], theta[3], theta[4]) for theta in samples])
...
if compute_BAO_predictions:
    ...
    def comoving_distance_Hubble_distance_from_z(Omega_m, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0, A, z_1, z):
        """
        Calculate the comoving distance and Hubble distances in Mpc from redshift z using the Hubble distance.
        """
        h = H0 / 100.0
        comoving_distance_ = comoving_distance_vec(Omega_m, w_gamma/(h**2), w_nu/(h**2), w_nu_ur/(h**2), a_nr_sq, h*H_100, A/100*H_100, z_1, z)  # in m
        Hubble_distance_ = c / Hubble_factor(z, Omega_m, w_gamma/(h**2), w_nu/(h**2), w_nu_ur/(h**2), a_nr_sq, h*H_100, A/100*H_100, z_1)  # DH in m
        return comoving_distance_, Hubble_distance_  # Return in Mpc

    w_b_mean, Omega_m_mean, H0_mean, A_mean, z_1_mean = results_min_chi2.x
    ...
    comoving_distance_general, Hubble_distance_general = comoving_distance_Hubble_distance_from_z(
        Omega_m_mean, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0_mean, A_mean, z_1_mean, z_general
        )

    comoving_distance_BAO, Hubble_distance_BAO = comoving_distance_Hubble_distance_from_z(
        Omega_m_mean, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0_mean, A_mean, z_1_mean, z_BAO
        )
    ...
    with open("output/fQ_gravity_phen_"+phenomenological_function+"_bao_predictions.txt", "w") as f:
        f.write("#BAO predictions for the Phen model\n")
    ...
    with open("output/fQ_gravity_phen_"+phenomenological_function+"_general_predictions.txt", "w") as f:
        f.write("#General predictions for the phen model\n")
        f.write("# z, DM, DH, DV, DM/r_d, DH/r_d, DV/r_d\n")
...
if compute_BAO_significance_from_CMB:
    ...
    try:
        data_cmb = np.load("dynesty_results_phen_"+phenomenological_function+"_final_CMB_only.npz")
    ...
    for i, theta in enumerate(samples_cmb):
        ...
        BAO_predictions[i] = model_predictions(z_DESI, omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, h*H_100, A/100*H_100, z_1, r_d)
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_phen_"+phenomenological_function+"_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_phen_"+phenomenological_function+"_final_DESI_only.npz")
    ...
    H0_A_rd = samples_all[:,1] + samples_all[:,2]
    omega_m_mean = np.average(samples_all[:,0], weights=weights_all)
    H0_A_rd_mean = np.average(H0_A_rd, weights=weights_all)
    covariance_omega_m_H0_A_rd = np.cov(samples_all[:,0], H0_A_rd, aweights=weights_all)
    ...
    covariance_omega_m_H0_rd = np.cov(samples_cmb[:,1], H0_rd_cmb, aweights=weights_cmb)

    mean_diff = np.array([omega_m_mean - omega_m_cmb_mean, H0_A_rd_mean - H0_rd_cmb_mean])
    total_covariance = covariance_omega_m_H0_A_rd + covariance_omega_m_H0_rd
...
if compute_BAO_significance_parameter_space_noDESI:
    ...
    try:
        ...
        samples_all = data_all['samples']
        weights_all = data_all['weights']
        data_cmb = np.load("dynesty_results_phen_"+phenomenological_function+"_final_no_desi.npz")
        samples_cmb = data_cmb['samples']
        weights_cmb = data_cmb['weights']
    except FileNotFoundError:
        raise FileNotFoundError("All-data Nested sampling results file not found. Please run nested sampling with all data first.")
