...
def log_likelihood(theta, use_desi=True, use_h0=True, use_cc=True, use_cmb=True, desi_only=False):
    ...
    if use_desi:
        if not desi_only:
            ...
            predictions = model_predictions(z_DESI, omega_m, 0, 0, 0, a_nr_sq, 1000*H0_rd, 1.0)
...
def chi_squared(theta, print_output=False):
    ...
    # SNII H_0
    chi2_SNII = ((H0 + 0.0 - H0_SNII) / eH0_SNII)**2

    # SBF H_0
    chi2_SBF = ((H0 + 0.0 - H0_SBF) / eH0_SBF)**2

    # Maser H_0
    chi2_maser = ((H0 + 0.0 - H0_maser) / eH0_maser)**2

    # Cosmic Chronometer (CC) data
    H_z_predicted = solve_Friedmann_vec(omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, z_CC) * H0
...
def prior_transform(u):
    ...
    if not nested_sampling_data == "desi_only":
        ...
        H0_rd = 5000 + 8000 * u[1]            # [0.05, 1.0] in units of 1000 km/s/Mpc
...
if perform_nested_sampling:
    ...
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_exp_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_exp_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_exp_final_desi_only.npz", samples=samples, weights=weights)
    else:
    # Save samples, weights, and log_posteriors to a .npz file
        np.savez("dynesty_results_exp_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
...
def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0):
    ...
    def integrand(a):
        ...
        E = solve_Friedmann(Omega_bc, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, z, bracket=[0.1, 1e10])
        return 1.0 / (a * E)
...
if compute_Universe_age:
    try:
        data = np.load("dynesty_results_exp_final.npz")
...
if compute_BAO_predictions:
    ...
    with open("output/fQ_gravity_exp_bao_predictions.txt", "w") as f:
    ...
    with open("output/fQ_gravity_exp_general_predictions.txt", "w") as f:
...
if compute_BAO_significance_from_CMB:
    try:
        data_cmb = np.load("dynesty_results_exp_final_CMB_only.npz")
    ...
    print("Mean BAO predictions from CMB-only chains:", mean_BAO_predictions)
    print("Covariance of BAO predictions from CMB-only chains:\n", cov_BAO_predictions)
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_exp_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_exp_final_desi_only.npz")
