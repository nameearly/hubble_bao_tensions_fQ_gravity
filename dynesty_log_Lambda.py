...
def Friedmann(E, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z):
    Omega_alpha = 1 - Omega_bc - Omega_gamma - Omega_nu - Omega_Lambda
    return E**2 + Omega_alpha * np.log(E) - Omega_alpha - Omega_bc * (1 + z)**3 - Omega_gamma * (1 + z)**4 - Omega_nu_ur * np.sqrt(1 + 1/((1 + z)**2 * a_nr_sq)) * (1 + z)**4 - Omega_Lambda
...
def prior_transform(u):
    ...
    if not nested_sampling_data == "desi_only":
        ...
        H0_rd = 5000 + 8000 * u[2]           # [0.05, 0.10] in units of 1000 km/s/Mpc
        if omega_m + omega_Lambda >= 1.0:
        # Return an impossible value (outside prior)
            return [0.01, 0.0, 0.05]
...
if perform_nested_sampling:
    ...
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_log_Lambda_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_log_Lambda_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_log_Lambda_final_desi_only.npz", samples=samples, weights=weights)
    else:
    # Save samples, weights, and log_posteriors to a .npz file
        np.savez("dynesty_results_log_Lambda_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
...
def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, Omega_Lambda, a_nr_sq, H0):
    ...
    integral, _ = quad(integrand, 1e-8, 1)
...
if compute_Universe_age:
    try:
        data = np.load("dynesty_results_log_Lambda_final.npz")
...
if compute_BAO_predictions:
    ...
    with open("output/fQ_gravity_log_Lambda_bao_predictions.txt", "w") as f:
    ...
    with open("output/fQ_gravity_log_Lambda_general_predictions.txt", "w") as f:
...
if compute_BAO_significance_from_CMB:
    try:
        data_cmb = np.load("dynesty_results_log_Lambda_final_CMB_only.npz")
    ...
    dof = len(Data_DESI) #- samples_cmb.shape[1]  # degrees of freedom
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_log_Lambda_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_log_Lambda_final_desi_only.npz")
