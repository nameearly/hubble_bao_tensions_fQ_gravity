...
def Friedmann(E, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z):
    Omega_alpha = (1 - Omega_bc - Omega_gamma - Omega_nu - Omega_Lambda)/rescaling
    inverse_E2 = 1 / E**2
    return E**2 - Omega_alpha/(np.cosh(inverse_E2))**2 * inverse_E2 - 0.5 * Omega_alpha * np.tanh(inverse_E2) - Omega_bc * (1 + z)**3 - Omega_gamma * (1 + z)**4 - Omega_nu_ur * np.sqrt(1 + 1/((1 + z)**2 * a_nr_sq)) * (1 + z)**4 - Omega_Lambda

def solve_Friedmann(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z, bracket=[0.1, 1e5]):
    #Omega_Lambda_LCDM = 1 - Omega_bc - Omega_gamma - Omega_nu
    E_guess = np.sqrt(Omega_bc * (1 + z)**3 +  1 - Omega_bc)  # LCDM guess
...
def log_prior(theta):
    ...
    if not (0.0 < omega_Lambda < 1.0): return -np.inf
...
def log_likelihood(theta, use_desi=True, use_h0=True, use_cc=True, use_cmb=True, desi_only=False):
    ...
    if use_desi:
        if not desi_only:
            ...
            predictions = model_predictions(z_DESI, omega_m, 0, 0, 0, omega_Lambda, a_nr_sq, 1000*H0_rd, 1.0)
...
def prior_transform(u):
    ...
    if not nested_sampling_data == "desi_only":
        ...
        H0_rd = 5000 + 8000 * u[2]           # [50.0, 100.0]
        if omega_m + omega_Lambda >= 1.0:
        # Return an impossible value (outside prior)
            return [0.01, 0.0, 50.0]
...
if perform_nested_sampling:
    ...
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_tanh_Lambda_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_tanh_Lambda_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_tanh_Lambda_final_desi_only.npz", samples=samples, weights=weights)
    else:
    # Save samples, weights, and log_posteriors to a .npz file
        np.savez("dynesty_results_tanh_Lambda_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)

def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, Omega_Lambda, a_nr_sq, H0):
    h = H0 / 100.0
    def integrand(a):
        z = 1 / a - 1
        E = a * solve_Friedmann(Omega_bc, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, Omega_Lambda, a_nr_sq, z, bracket=[0.1, 1e20])
        return 1.0 / E
...
if compute_Universe_age:
    try:
        data = np.load("dynesty_results_tanh_Lambda_final.npz")
    ...
    with multiprocessing.Pool(processes=nprocs) as pool:
        ages = pool.starmap(age_of_universe, [(theta[1], w_gamma, w_nu, w_nu_ur, theta[2], a_nr_sq, theta[3]) for theta in samples])
...
if compute_BAO_predictions:
    ...
    with open("output/fQ_gravity_tanh_Lambda_bao_predictions.txt", "w") as f:
    ...
    with open("output/fQ_gravity_tanh_Lambda_general_predictions.txt", "w") as f:
...
if compute_BAO_significance_from_CMB:
    try:
        data_cmb = np.load("dynesty_results_tanh_Lambda_final_CMB_only.npz")
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_tanh_Lambda_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_tanh_Lambda_final_desi_only.npz")
