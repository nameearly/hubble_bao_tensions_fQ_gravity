...
def Friedmann(E, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z):
    lambda_ = 0.5 + lambertw(-(Omega_bc + Omega_gamma + Omega_nu + Omega_Lambda)/(2*np.sqrt(np.e))).real
    return (E**2 - 2*lambda_) * np.exp(lambda_/E**2) - Omega_bc * (1 + z)**3 - Omega_gamma * (1 + z)**4 - Omega_nu_ur * np.sqrt(1 + 1/((1 + z)**2 * a_nr_sq)) * (1 + z)**4 - Omega_Lambda

def solve_Friedmann(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z, bracket=[0.1, 1e5]):
    E_guess = np.sqrt(Omega_bc*(1+z)**3 + 1 - Omega_bc)  # Initial guess for E
    sol = root_scalar(Friedmann, args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z), bracket=bracket, x0=E_guess)
    return sol.root if sol.converged else 0.3

def integrand_rec(u, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq):
    z = np.sinh(u)
    E = solve_Friedmann(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z)
    return np.cosh(u) / E

def integrand(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq):
    E = solve_Friedmann(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z)
    return 1 / E

from scipy.integrate import quad

def comoving_distance_arcsinh(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, H0, z_star):
    integral, _ = quad(integrand_rec, 0, np.arcsinh(z_star), args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq))
    return c / H0 * integral  # Return in Mpc

def comoving_distance(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, H0, z):
    integral, _ = quad(integrand, 0, z, args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq))
    return c / H0 * integral  # Return in Mpc

comoving_distance_vec = np.vectorize(comoving_distance, excluded=['Omega_bc', 'Omega_gamma', 'Omega_nu', 'Omega_nu_ur', 'Omega_Lambda', 'a_nr_sq', 'H0'])
solve_Friedmann_vec = np.vectorize(solve_Friedmann, excluded=['Omega_bc', 'Omega_gamma', 'Omega_nu', 'Omega_nu_ur', 'Omega_Lambda', 'a_nr_sq'])

def model_predictions(z, Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, H0, r_d):
    DM_array = comoving_distance_vec(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, H0, z)
    DH_array = c / (H0 * solve_Friedmann_vec(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, Omega_Lambda, a_nr_sq, z))
...
def log_prior(theta):
    w_b, omega_m, omega_Lambda, H0 = theta
    if not (0.020 < w_b < 0.025): return -np.inf
    if not (0.01 < omega_m < 0.49): return -np.inf
    if not (0.00 < omega_Lambda < 1.00): return -np.inf
...
def theta_star(w_b, w_bc, omega_Lambda, h):
    ...
    DM_rec = comoving_distance_arcsinh(w_bc / h**2, w_gamma / h**2, w_nu / h**2, w_nu_ur / h**2, omega_Lambda, a_nr_sq, h * H_100, z_rec)
...
def chi2_cmb(w_m, w_b, omega_Lambda, H0):
    
    theta_star_value = 100*theta_star(w_b, w_m, omega_Lambda, H0/100)
...
def log_likelihood(theta, use_desi=True, use_h0=True, use_cc=True, use_cmb=True, desi_only=False):
    if not desi_only:
        ...
        omega_m, omega_Lambda, H0_rd = theta
    ...
    if use_cmb:
        chi2_CMB = chi2_cmb(w_m, w_b, omega_Lambda, H0)
        chi2_total += chi2_CMB
    ...
    if use_desi:
        if not desi_only:
            ...
            predictions = model_predictions(z_DESI, omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, omega_Lambda, a_nr_sq, h*H_100, r_d)
        else: 
            predictions = model_predictions(z_DESI, omega_m, 0, 0, 0, omega_Lambda, a_nr_sq, 1000*H0_rd, 1)
    ...
    # Cosmic Chronometers
    if use_cc:
        H_z_predicted = H0 * solve_Friedmann_vec(omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, omega_Lambda, a_nr_sq, z_CC)
...
def prior_transform(u):
    ...
    if not nested_sampling_data == "desi_only":
        ...
        omega_Lambda = 0.00 + 1.00 * u[2]    # [0.00, 1.00]
        H0 = 50.0 + 50.0 * u[3]              # [50.0, 100.0]
        if omega_m + omega_Lambda >= 1.0:
        # Return an impossible value (outside prior)
            return [0.020, 0.01, 0.0, 50.0]

        return [w_b, omega_m, omega_Lambda, H0]
    else:
        omega_m = 0.01 + 0.48 * u[0]         # [0.01, 0.49]
        omega_Lambda = 0.00 + 1.00 * u[1]    # [0.00, 1.00]
        H0_rd = 5000 + 8000 * u[2]            # [0.05, 1.0] in units of 100 km/s/Mpc
        if omega_m + omega_Lambda >= 1.0:
        # Return an impossible value (outside prior)
            return [0.01, 0.0, 5000]

        return [omega_m, omega_Lambda, H0_rd]
...
def chi_squared(theta, print_output=False):
    ...
    H_z_predicted = solve_Friedmann_vec(omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, omega_Lambda, a_nr_sq, z_CC) * H0
...
def minimize_chi2():
    ...
    initial_guess = [0.022, 0.3, 0.1, 70.0]  # Initial guess for [w_b, omega_m, omega_Lambda, H0]
    result = minimize(chi_squared, initial_guess, method='Nelder-Mead')

    if result.success:
        fitted_params = result.x
        print("Fitted parameters:")
        print(f"w_b: {fitted_params[0]:.6f}, omega_m: {fitted_params[1]:.6f}, omega_Lambda: {fitted_params[2]:.6f}, H0: {fitted_params[3]:.6f}")
...
if not nested_sampling_data == "desi_only":
    ...
    ndim = 3
nprocs = 16  # or however many CPU cores you want to use
...
if perform_nested_sampling:
    ...
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_exp_Lambda_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_exp_Lambda_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_exp_Lambda_final_desi_only.npz", samples=samples, weights=weights)
    else:
    # Save samples, weights, and log_posteriors to a .npz file
        np.savez("dynesty_results_exp_Lambda_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)

def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, a_nr_sq, Omega_Lambda, H0):
    h = H0 / 100.0
    
    # 1. Define the integrand
    def integrand(a):
        z = 1.0 / a - 1.0
        # solve_Friedmann returns E(z) = H(z)/H0
        # The integrand for time is 1 / (a * H(z))
        # This simplifies to 1 / (a * H0 * E(z)) 
        # We factor out H0 outside the integral, so we return 1/(a * E)
        E = solve_Friedmann(Omega_bc, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, Omega_Lambda, a_nr_sq, z, bracket=[0.1, 1e10])
        return 1.0 / (a * E)

    # 2. Set a small lower bound to avoid z -> infinity singularity
    # a=1e-7 corresponds to z ~ 10,000,000 (well into radiation dominance)
    a_min = 1e-7
    
    # 3. Integrate with a slightly higher limit if necessary
    integral, error = quad(integrand, a_min, 1.0, limit=200)

    # 4. Conversion Factors
    # integral is in units of 1/H0
    t_0 = integral / H0 # becomes Mpc / (km/s)
    
    # Convert Mpc to km: 1 Mpc = 3.086e19 km
    # But easier: 1/H0 where H0 is in km/s/Mpc gives time in (Mpc*s)/km
    # conversion factor: 977.8 Gyr * km/s/Mpc approx
    
    # Your explicit conversion:
    t_0 = t_0 * Mpc / (1e3)  # Convert to seconds (Mpc in meters / 1000 => km)
...
if compute_Universe_age:
    try:
        data = np.load("dynesty_results_exp_Lambda_final.npz")
    ...
    with multiprocessing.Pool(processes=nprocs) as pool:
        ages = pool.starmap(age_of_universe, [(theta[1], w_gamma, w_nu, w_nu_ur, a_nr_sq, theta[2], theta[3]) for theta in samples])
...
if compute_BAO_predictions:
    ...
    def comoving_distance_Hubble_distance_from_z(Omega_m, w_gamma, w_nu, w_nu_ur, Omega_Lambda, a_nr_sq, H0, z):
        """
        Calculate the comoving distance and Hubble distances in Mpc from redshift z using the Hubble distance.
        """
        h = H0 / 100.0
        comoving_distance_ = comoving_distance_vec(Omega_m, w_gamma/(h**2), w_nu/(h**2), w_nu_ur/(h**2), Omega_Lambda, a_nr_sq, h*H_100, z)  # in Mpc
        Hubble_distance_ = c / (h*H_100*solve_Friedmann_vec(Omega_m, w_gamma/(h**2), w_nu/(h**2), w_nu_ur/(h**2), Omega_Lambda, a_nr_sq, z))  # DH in Mpc
        return comoving_distance_, Hubble_distance_  # Return in Mpc

    w_b_mean, Omega_m_mean, Omega_Lambda_mean, H0_mean = results_min_chi2.x
    ...
    comoving_distance_general, Hubble_distance_general = comoving_distance_Hubble_distance_from_z(
        Omega_m_mean, w_gamma, w_nu, w_nu_ur, Omega_Lambda_mean, a_nr_sq, H0_mean, z_general
        )

    comoving_distance_BAO, Hubble_distance_BAO = comoving_distance_Hubble_distance_from_z(
        Omega_m_mean, w_gamma, w_nu, w_nu_ur, Omega_Lambda_mean, a_nr_sq, H0_mean, z_BAO
        )
    ...
    with open("output/fQ_gravity_exp_Lambda_bao_predictions.txt", "w") as f:
    ...
    with open("output/fQ_gravity_exp_Lambda_general_predictions.txt", "w") as f:
...
if compute_BAO_significance_from_CMB:
    try:
        data_cmb = np.load("dynesty_results_exp_Lambda_final_CMB_only.npz")
    ...
    for i, theta in enumerate(samples_cmb):
        ...
        w_m = omega_m * h**2
        r_d = 147.05 * Mpc * (w_b/0.02236)**-0.13 * (w_m/0.1432)**-0.23 * (N_eff/3.04)**-0.1
        BAO_predictions[i] = model_predictions(z_DESI, omega_m, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, omega_Lambda, a_nr_sq, h*H_100, r_d)
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_exp_Lambda_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_exp_Lambda_final_desi_only.npz")
    ...
    H0_rd_mean = np.average(samples_all[:,2], weights=weights_all)
    covariance_omega_m_H0_rd = np.cov(samples_all[:,0], samples_all[:,2], aweights=weights_all)

    wb_values = samples_cmb[:,0]
    Omega_bc_values = samples_cmb[:,1]
    Omega_Lambda_values = samples_cmb[:,2]
    h_values = samples_cmb[:,3] / 100.0
    h_sq_values = h_values * h_values
    
    rd = 147.05 * (wb_values/0.02236)**-0.13 * ((Omega_bc_values*h_sq_values)/0.1432)**-0.23 * (N_eff/3.04)**-0.1
    H0_rd_cmb = samples_cmb[:,3] * rd 
...
if compute_BAO_significance_parameter_space_noDESI:
    ...
    #omega_Lambda_values = samples_all[:,1]
    H0_rd_values = samples_all[:,2]
    ...
    h_values = samples_no_desi[:,3] / 100.0
    h_sq_values = h_values * h_values
