...
def solve_Friedmann(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, z):
    ...
    sol = root_scalar(Friedmann, args=(Omega_bc, Omega_gamma, Omega_nu, Omega_nu_ur, a_nr_sq, z), bracket=[1e-5, 1e20], x0=E_guess)
...
if perform_nested_sampling:
    ...
    if nested_sampling_data == "CMB_only":
        np.savez("dynesty_results_log_final_CMB_only.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "no_desi":
        np.savez("dynesty_results_log_final_no_desi.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)
    elif nested_sampling_data == "desi_only":
        np.savez("dynesty_results_log_final_desi_only.npz", samples=samples, weights=weights)
    else:
    # Save samples, weights, and log_posteriors to a .npz file
        np.savez("dynesty_results_log_final.npz", samples=samples, weights=weights, log_posteriors=log_posteriors)

def age_of_universe(Omega_bc, w_gamma, w_nu, w_nu_ur, a_nr_sq, H0):
    h = H0 / 100.0
    def integrand(a):
        z = 1 / a - 1
        E = a * solve_Friedmann(Omega_bc, w_gamma/h**2, w_nu/h**2, w_nu_ur/h**2, a_nr_sq, z)
        return 1.0 / E
...
if compute_Universe_age:
    try:
        if nested_sampling_data == "CMB_only":
            data = np.load("dynesty_results_log_final_CMB_only.npz")
        elif nested_sampling_data == "no_desi":
            data = np.load("dynesty_results_log_final_no_desi.npz")
        else:
            data = np.load("dynesty_results_log_final.npz")
...
if compute_BAO_predictions:
    ...
    with open("output/fQ_gravity_log_bao_predictions.txt", "w") as f:
    ...
    with open("output/fQ_gravity_log_general_predictions.txt", "w") as f:
...
if compute_BAO_significance_from_CMB:
    try:
        data_cmb = np.load("dynesty_results_log_final_CMB_only.npz")
    ...
    sigma_tension = norm.isf(p_value)
...
if compute_BAO_significance_from_noDESI:
    ...
    try:
        data_no_desi = np.load("dynesty_results_log_final_no_desi.npz")
...
if compute_BAO_significance_parameter_space_CMB:
    ...
    try:
        data_all = np.load("dynesty_results_log_final_desi_only.npz")
