...
Each file runs a different model. We include the files to run LCDM, the phenomenological models, and the three f(Q) gravity models (Exp, Log, Tanh) along with their extensions including the cosmological constant.

At the start of each file, we have several flags to control what we want to compute. 

1. "compute_minimum_chi2": this computes the minimum chi squared contributions reported in table V.
2. "perform_nested_sampling": this does the nested sampling MCMC to compute the Bayesian Evidence and save the samples to an npz file for later use. These samples are required for all the following flags.
